...
def _behavior_priors(
    *,
    ...
    maintain = max(0.2, 1.0 - maneuver - reversal - drift - slow_maneuver)
    weights = {
...
def predict_dark_vessel(
    ...
    desired_scenarios = max(
        5,
        min(6, 5 + len(missing) + int(age_s // 900)),
    )
    ...
    wave_height = float(wave.get("height_m", 0.0)) if wave.get("available") else 0.0
    ...
    for _ in range(SAMPLE_COUNT):
        ...
        if behavior == "course_reversal":
            desired_bearing = (
                base_bearing + 180 + rng.gauss(0, 18 + heading_sigma * 0.2)
            ) % 360
        elif behavior == "maneuver":
            offset = max(
                -120.0,
            ...
            desired_bearing = (base_bearing + offset) % 360
            ...
            desired_bearing = (
                base_bearing + rng.gauss(0, heading_sigma * 0.35)
            ) % 360
        ...
        for step in range(steps):
            if behavior == "maintain_course":
                desired_bearing = (
                    desired_bearing
                    + rng.gauss(0, max(0.08, heading_sigma / 160))
                ) % 360
            elif behavior == "slow_maneuver":
                desired_bearing = (
                    desired_bearing
                    + math.sin((step + 1) * 0.55)
            ...
            sample_turn = max(
                ...
                min(
                    max_turn,
                    sample_turn
                    + (desired_turn - sample_turn) * 0.42
                    + rng.gauss(0, turn_noise),
                ),
            )
            bearing = (bearing + sample_turn * STEP_MINUTES) % 360

            if behavior == "drift":
                speed = max(0.0, speed * 0.58)
            else:
                speed = max(
                    0.0,
                    speed
                    + (target_speed - speed) * 0.25
                    + rng.gauss(0, speed_sigma * 0.035),
                )
            ...
            if current_speed > 0:
                current_bearing = (
                    math.degrees(math.atan2(current_east, current_north)) % 360
                )
                sampled_current = max(
                    0.0,
                    rng.gauss(current_speed, max(0.02, current_speed * 0.1)),
                )
                next_lat, next_lon = _destination(
                    next_lat,
                    next_lon,
            ...
            if wind_speed > 0:
                leeway_factor = windage if behavior == "drift" else windage * 0.2
                sampled_leeway = max(
                    0.0,
                    rng.gauss(
                ...
                next_lat, next_lon = _destination(
                    ...
                    sampled_leeway * STEP_MINUTES * 60,
                )
        ...
        samples.append({"path": path, "behavior": behavior})

    scenarios = _cluster(samples, desired_scenarios, (lat, lon))
    terrain_here = context.terrain_status(lat, lon)
    behavior_counts = {
        behavior: sum(sample["behavior"] == behavior for sample in samples)
    ...
    return {
        "model": "monte_carlo_navigation_v3",
